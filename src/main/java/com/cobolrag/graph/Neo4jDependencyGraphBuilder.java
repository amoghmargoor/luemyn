...
public class Neo4jDependencyGraphBuilder implements AutoCloseable {
    ...
    public void buildRelationships() {
        try (Session session = driver.session()) {
            // Find all programs
            Result result = session.run("MATCH (p:Program) RETURN p.name AS name, p.filePath AS filePath");

            while (result.hasNext()) {
                Record record = result.next();
                String programName = record.get("name").asString();
                String filePath = record.get("filePath").asString();

                // Parse the program again to extract dependencies
                Program program = cobolParser.parseFile(new java.io.File(filePath), cobolFormat);
